...
class AncaratPriceController extends Controller
{
     ...
     * Trả về danh sách tất cả sản phẩm Ancarat + giá current
     */
    public function currentPrice(): JsonResponse
    {
        $prices = SilverPrice::where('source', 'ancarat')->get();
        ...
        $byUnit = [];
        foreach ($prices as $p) {
            // Normalize: 'LUONG_A4' -> 'LUONG', 'KG_K4' -> 'KG'
            $normKey = preg_replace('/_[A-Z0-9]+$/', '', $p->unit); // strip suffix
            $key = $normKey ?: $p->unit;
            $byUnit[$key] = [
                'unit'           => $p->unit,
        ...
        // Lấy bản ghi mới nhất để hiển thị updated_at
        $latest = SilverPrice::where('source', 'ancarat')
