...
<style>
  ...
  .thumb-slot img { max-width: 160px; border-radius: 10px; border: 2px solid #e2e6f0; }
  .thumb-slot small { display: block; margin-top: 4px; }
  .thumb-preview-box { position: relative; display: inline-block; }
  .thumb-preview-box img { max-width: 160px; border-radius: 10px; border: 2px solid #667eea; }
  .thumb-remove {
          ...
          <div class="card post-form-card">
            <div class="card-body">
              ...
              <div class="post-form-header">
                <h5>✏️ Cập nhật bài viết</h5>
                <a href="{{ backendRoute('post.index') }}" class="btn btn-back">
                  <i class="mdi mdi-arrow-left"></i> Quay lại
                </a>
              </div>

              <form class="post-form" action="{{ backendRoute('post.update', ['post' => $data->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                  ...
                  <div style="max-width: 50%;">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="title" id="title" required
                             value="{{ oldInput(old('title'), $data->title) }}" placeholder="Nhập tiêu đề bài viết">
                    </div>

                    <div class="form-group">
                      <label class="col-form-label">Slug (URL SEO)</label>
                      <input type="text" class="form-control" name="slug" id="slug" readonly
                             value="{{ oldInput(old('slug'), $data->slug) }}" placeholder="Tự tạo từ tiêu đề">
                    </div>

                    <div class="form-group">
                      <label class="col-form-label">Mô tả ngắn (excerpt)</label>
                      <textarea name="excerpt" class="form-control" rows="3"
                                placeholder="Tóm tắt ngắn hiển thị ở danh sách bài viết">{{ oldInput(old('excerpt'), $data->excerpt) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="category_id" class="form-control" style="max-width: 280px;">
                        ...
                        @php $currentCat = oldInput(old('category_id'), $data->category_id); @endphp
                    ...
                    <div class="form-group">
                      <div class="custom-checkbox-featured">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1"
                               {{ oldInput(old('is_featured'), $data->is_featured) ? 'checked' : '' }}>
